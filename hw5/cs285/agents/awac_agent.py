...
class AWACAgent(DQNAgent):
    ...
    def get_qvals(self, critic, obs, action):
        ...
        if isinstance(action, int):
            q_values = qa_t_values[:, action]
        else:
            q_values = torch.gather(qa_t_values, 1, action.long().unsqueeze(1)).squeeze(1)
    ...
    def estimate_advantage(self, ob_no, ac_na, re_n, next_ob_no, terminal_n, n_actions=10):
        ...
        if self.agent_params['discrete']:
            for i in range(self.agent_params['ac_dim']):
                prob = dist.probs[:, i]
                q_i = self.get_qvals(self.exploitation_critic, ob_no, i)
                vals.append(prob * q_i)
        else:
            assert False, 'TA said there would be no continuous environments.'
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        if (self.t > self.learning_starts
            ...
            actor_loss = self.eval_policy.update(ob_no, ac_na, adv)
